...
export default class PositionRepositoryDatabase implements PositionRepository {
  ...
  async savePosition(position: Position): Promise<void> {
    await this.connection.query("insert into cccat17.position (ride_id, lat, long, date) values ($1, $2, $3, $4)", [position.rideId, position.lat, position.long, position.date])
  }
